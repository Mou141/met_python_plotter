"""Gets the specified forecast layer at all available timesteps."""
import argparse
import sys
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Downloads the specified forecast layer at all available timesteps."
    )
...
def main(api_key: str, layer_name: str):
    ...
    try:
        for t, img in m.get_all_forecast_layers_of_type(layer_name):
            fname = f"{layer_name}_{t}.png"

            print(f"Saving to '{fname}...")
            img.save(fname, format="PNG")

    except (OSError, ValueError) as e:
        print(f"Error while retrieving forecast layer images: '{e}'", file=sys.stderr)
        sys.exit(1)
